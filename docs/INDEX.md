...
The public Windows v0.2.0 Preview requires no GitHub login. Speech weights, optional Ollama/text models and third-party VST plugins are separate. The online helper downloads the existing app payload; the app's update feed remains unconfigured. Read the [dependency notices](BUNDLED-NOTICES.md) before redistribution.

공개 Windows v0.2.0 Preview는 GitHub 로그인이 필요하지 않습니다. 음성 가중치·선택적 Ollama/텍스트 모델·외부 VST는 별도입니다. 온라인 도우미는 기존 앱 데이터를 받으며 앱의 업데이트 feed는 미구성 상태입니다. 재배포 전 [의존성 고지](BUNDLED-NOTICES.md)를 확인하세요.
...
- [v0.2.0 release and verification / v0.2.0 릴리즈·검증](releases/v0.2.0.md)
