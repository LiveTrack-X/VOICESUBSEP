...
> **역사적 설계 원문을 보존한 문서입니다.** 아래의 “현재·미구현”은 작성 당시 상태입니다. 0.3.0 대상에는 캐시 전용 로컬 Whisper+Nemotron 지속 세션과 라이브 자막·읽기 전용 OBS 송출 경로가 구현되었으며, 개별 WASAPI 출력·ASIO·자동 드리프트 보정은 별도 범위입니다. 번역·AI 자동 요약은 제거했습니다. 현행 사용법은 [사용자 가이드](USER-GUIDE.md#8-라이브-자막마이크시스템-소리-녹음)·[기능 현황](FEATURE-STATUS.md), 게시·설치 검증은 [0.3.0 기록](releases/v0.3.0.md)을 우선합니다.

상태: **2026-09-24의 설계 제안 기록**. 현재 상태 갱신: 2026-09-25.

개발 소스에는 마이크·시스템 소리의 MediaRecorder 녹음, 약 1초 단위 IndexedDB 보관·복구, 소스별 다운로드, **종료 후** 혼합본을 새 프로젝트로 분석하는 흐름과 인터뷰/회의록 편집이 추가되었습니다. 데스크톱 소스에는 사용자 동작과 권한 확인을 거치는 Windows loopback 캡처 경로가 있습니다. 사용법과 제한은 [현재 편집 워크플로](EDITING-WORKFLOWS.md)를 우선합니다. 새 설치본으로 배포하거나 장시간 실기기 녹음 품질을 확인했다는 의미는 아닙니다.

아래 본문의 “제안·미구현” 설명과 데이터 계약은 조사 당시의 설계를 보존합니다. 아래에 제안한 별도 WASAPI 네이티브 녹음 워커, 녹음 중 스트리밍 ASR, 화자 ID 고정, 외부 오디오 시계 드리프트 보정, 자동 장치 재연결은 현재 구현하지 않았습니다. 시작 오프셋 메타데이터 저장만으로 해당 시간 동기화 계약을 충족했다고 보지 않습니다.
