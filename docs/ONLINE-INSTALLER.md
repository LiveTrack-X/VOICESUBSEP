...
This guide targets the published **0.3.4 Windows 10/11 x64 online installer**. See the [0.3.4 release record](releases/v0.3.4.md) for hashes, public-file checks and local installation evidence. Public downloads require no GitHub account/token. The helper downloads a version-pinned app payload; it does not rebuild the application. Read the [dependency notices and distribution scope](BUNDLED-NOTICES.md) before redistribution.

공개된 Windows 10/11 x64용 **0.3.4 설치 안내**입니다. 해시·공개 파일 검사·로컬 설치 증거는 [0.3.4 릴리즈 기록](releases/v0.3.4.md)을 확인하세요. 공개 다운로드에 GitHub 계정·토큰은 필요 없으며, 도우미는 해당 버전의 앱 데이터를 받아 설치할 뿐 재빌드하지 않습니다. 소스 공개가 자유로운 재배포 허용을 뜻하지 않으므로 [의존성 고지·재배포 범위](BUNDLED-NOTICES.md)를 확인하세요.

**Online installer:** [VOICESUBSEP-0.3.4-Online-Setup-x64.exe](https://github.com/LiveTrack-X/VOICESUBSEP/releases/download/v0.3.4/VOICESUBSEP-0.3.4-Online-Setup-x64.exe) (166,912 bytes). Its anonymous download matched the verified local file. See the [release record](releases/v0.3.4.md) for the checksum and verification scope. Manual assembly remains an alternative.

**온라인 설치기:** [VOICESUBSEP-0.3.4-Online-Setup-x64.exe](https://github.com/LiveTrack-X/VOICESUBSEP/releases/download/v0.3.4/VOICESUBSEP-0.3.4-Online-Setup-x64.exe) (166,912바이트). 로그인 없이 받은 파일이 검증한 로컬 파일과 일치했습니다. 체크섬·검증 범위는 [릴리즈 기록](releases/v0.3.4.md)에 남겼습니다. 수동 조립 방식도 유지합니다.
...
The payload includes Python, FFmpeg, speech libraries, CUDA runtime libraries, **Silero VAD ONNX and the pinned RNNoise model**. Whisper/Nemotron weights and the NVIDIA driver are separate. RNNoise is optional and off by default; its bundled model needs no runtime download. Before live use, connect a short audio/video file and complete a local analysis with the same Whisper model and Nemotron enabled; file analysis downloads missing weights. Live mode only accepts those complete caches. Commercial VST3 effects are separate. Qwen is not included in the installer. Subtitle translation and automatic AI summaries remain removed. The helper's speed limit does not limit later model downloads.

Python·FFmpeg·음성 라이브러리·CUDA 런타임과 **Silero VAD ONNX·고정 RNNoise 모델은 포함**합니다. Whisper/Nemotron 가중치·NVIDIA 드라이버는 별도입니다. RNNoise는 기본 꺼짐인 선택 기능이며 포함된 모델을 실행 중 다시 받지 않습니다. 라이브 전에 짧은 음성·영상 파일을 연결하고 `음성 분석`에서 같은 Whisper 모델·로컬 Nemotron 분석을 한 번 완료해 없는 가중치를 받습니다. 라이브는 이 완성된 캐시만 읽습니다. 상용 VST3는 별도이고 Qwen은 설치본에 포함하지 않습니다. 자막 번역·AI 자동 요약은 제거 상태를 유지합니다. 도우미의 속도 제한은 이후 모델 다운로드에 적용되지 않습니다.

## Install / 설치

1. After publication is confirmed in the release record, download **`VOICESUBSEP-0.3.4-Online-Setup-x64.exe`** and run it. The Windows executable is Authenticode-unsigned; verify its source and published checksum.
...
1. 릴리즈 기록에서 게시를 확인한 뒤 **`VOICESUBSEP-0.3.4-Online-Setup-x64.exe`**를 실행합니다. Windows Authenticode 미서명이므로 출처와 공개 체크섬을 확인하세요.
...
```text
%LOCALAPPDATA%\VOICESUBSEP\InstallerCache\0.3.4
```
...
Publication/download/installation status for this guide belongs to [0.3.4](releases/v0.3.4.md). The historical [0.3.3](releases/v0.3.3.md), [0.3.2](releases/v0.3.2.md), [0.3.1](releases/v0.3.1.md), [0.3.0](releases/v0.3.0.md), [0.2.1](releases/v0.2.1.md) and [0.2.0](releases/v0.2.0.md) records remain unchanged and do not prove installation or GPU inference for a different build.

이 안내의 게시·다운로드·설치 상태는 [0.3.4 기록](releases/v0.3.4.md)을 따릅니다. 과거 [0.3.3](releases/v0.3.3.md)·[0.3.2](releases/v0.3.2.md)·[0.3.1](releases/v0.3.1.md)·[0.3.0](releases/v0.3.0.md)·[0.2.1](releases/v0.2.1.md)·[0.2.0](releases/v0.2.0.md) 기록은 그대로 보존하며 다른 빌드의 설치·GPU 추론 증거로 재사용하지 않습니다.

The app's **in-app updater is separate from this setup helper**. It uses a bundled Ed25519 public key to authenticate a GitHub release manifest and SHA256 to verify split files/payload, with explicit check/download/install and serial 80 Mbps transfers. Its incomplete individual downloads restart; only complete verified files are reused. This is manifest authentication, **not Windows Authenticode signing**. See [desktop updates](DESKTOP.md#사용자가-선택하는-업데이트).

**인앱 업데이트는 이 설치 도우미와 별도**입니다. 포함된 Ed25519 공개키로 GitHub 명세를 인증하고 SHA256로 조각·payload를 검증하며 확인·다운로드·설치를 직접 선택합니다. 순차 80Mbps이고 미완료 개별 파일은 다시 받으며 검증된 완료 파일만 재사용합니다. 이는 명세 인증이며 **Windows Authenticode 코드 서명은 아닙니다**. [업데이트 계약](DESKTOP.md#사용자가-선택하는-업데이트)을 참고하세요.
