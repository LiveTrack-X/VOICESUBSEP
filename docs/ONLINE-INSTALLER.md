...
**Published online installer:** download `VOICESUBSEP-0.2.0-Online-Setup-x64.exe` (166,912 bytes, about 167 KB) from the [release assets](https://github.com/LiveTrack-X/VOICESUBSEP/releases/tag/v0.2.0). Its anonymous download was checked against the published binary. The existing seven-file manual installer remains available. A complete NSIS installation and new GPU inference have not been performed for this addition.

**온라인 설치기 게시 완료:** [릴리즈 자산](https://github.com/LiveTrack-X/VOICESUBSEP/releases/tag/v0.2.0)의 `VOICESUBSEP-0.2.0-Online-Setup-x64.exe`를 받으세요(166,912바이트, 약 167KB). 비로그인으로 다시 받은 파일의 해시를 게시 바이너리와 대조했습니다. 기존 7개 파일 수동 방식도 유지합니다. 이번 추가에서 전체 NSIS 설치·새 GPU 추론은 수행하지 않았습니다.

Online EXE SHA256 / 온라인 EXE SHA256:

```text
c4ca8cb9acb43fc1c8361af10018eef25567ac5814ab7c6af7b31ddc547f0b62
```
...
| Download / 다운로드 | Existing installer EXE plus 2,416,033,719-byte app payload, about 2.42 GB / 기존 설치 EXE와 2,416,033,719바이트 앱 데이터, 약 2.42GB |
...
Reported checks so far: anonymous download of the original 1,045,697-byte NSIS EXE; .NET assembly of the 2,416,033,719-byte payload using three existing local parts; the online UI's `--smoke` path; and 15 downloader-core tests, including timed throttling and connection interruption. These checks do not cover a complete fresh internet download of all large parts, an actual NSIS installation, device capture, or a new full GPU inference run. Final artifact identity and additional checks belong in the [release record](releases/v0.2.0.md).

현재 확인된 범위는 원래 NSIS EXE 1,045,697바이트의 비로그인 다운로드, 기존 로컬 조각 3개를 이용한 .NET 전체 payload 2,416,033,719바이트 조립, 온라인 UI의 `--smoke`, 시간 측정 속도 제한·연결 중단을 포함한 다운로더 코어 검사 15개입니다. 대형 조각 전체의 신규 인터넷 다운로드·실제 NSIS 설치·장치 녹음·새 전체 GPU 추론을 확인한 것은 아닙니다. 최종 자산 식별 정보와 추가 검사는 [릴리즈 기록](releases/v0.2.0.md)에 남깁니다.
