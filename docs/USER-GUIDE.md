...
대상: **Windows x64, v0.2.0 Preview 시험 릴리즈**. 비공개 저장소 접근 권한이 필요합니다. 현재 설치 파일 생성·설치·GitHub 게시를 준비 중이며 확인 결과는 [릴리즈 기록](releases/v0.2.0.md)에 따로 기록합니다. 이 가이드의 설치 절차만으로 배포 완료를 뜻하지 않습니다.
