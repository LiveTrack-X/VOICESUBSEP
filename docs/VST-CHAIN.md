...
2026-09-24 개발본에 추가한 선택 기능이다. **Windows x64 VST3 효과**를 최대 4개까지 연결해 Whisper 앞에서 처리한다. 원본 미디어와 미디어 내보내기 오디오는 바뀌지 않는다. v0.2.0 설치본의 런타임 포함·검증·게시 여부는 [릴리즈 기록](releases/v0.2.0.md)을 따른다. 상용 VST3 플러그인은 설치본에 포함하지 않는다.
...
- 플러그인은 서버와 분리된 숨김 작업 프로세스에서 실행한다. 한 서버에서 동시에 하나만 실행하고, 충돌·정지·취소 시 소유한 작업 프로세스를 회수한다. 프로세스 분리는 충돌 격리이며 보안 샌드박스가 아니다.
- 전체 녹음을 메모리에 올리지 않고 청크 단위로 처리한다. 긴 작업의 파생 WAV는 RF64를 지원하며 디스크 여유 공간이 필요하다.
- 처리 시작에 체인을 한 번 초기화하고 청크 사이에는 상태를 유지한다. 각 플러그인의 보고 지연을 읽어 순차 자동 보정하며, 마지막 버퍼를 방출한 뒤 원본과 같은 샘플 수로 마친다. 다음 처리에서 변경된 체인/파라미터를 다시 초기화하므로 지연도 다시 확인한다. 지연을 잘못 보고하는 플러그인의 싱크까지 보장하지는 않는다.
- A/B 결과에는 플러그인별 지연과 합계를 샘플·밀리초로 표시한다. 처리 중 지연이 바뀌거나 실제 제거된 프레임 수가 보고 지연과 일치하지 않으면 결과를 적용하지 않고 오류로 끝낸다. 고정 47 ms 값을 일괄 차감하지 않는다.
