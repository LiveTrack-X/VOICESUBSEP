...
if (!['pack', 'installer', 'installer-split'].includes(mode) || process.argv.length !== 3) throw new Error('Use: node scripts/desktop-build.mjs pack|installer|installer-split');
...
const target = mode === 'pack' ? ['--dir'] : [mode === 'installer-split' ? 'nsis-web' : 'nsis'];
const child = spawn(process.execPath, [path.join(root, 'node_modules/electron-builder/cli.js'), '--config', 'desktop/electron-builder.cjs', '--win', ...target, '--x64', '--publish', 'never'], {
